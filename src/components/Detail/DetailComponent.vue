<template>
  <section class="w-full sm:max-w-5xl px-5 py-10 mx-auto">
      ...
      <div class="flex items-center">
        <div class="relative mr-4">
          <button
            ...
            class="w-[60px] h-[60px] bg-transparent border border-gray-400 hover:border-blue-500 text-gray-400 hover:text-blue-500 rounded-full text-lg font-bold"
          >
          ...
          <div data-cy="sort-parent" v-show="isFilterDropdownOpen" class="absolute min-w-[200px] z-1 shadow-md bg-white rounded-lg mt-3">
            ...
            <a data-cy="sort-unfinished" @click="setTodoFilter('not-done')" class="px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-gray-200">
              ...
              <font-awesome-icon v-show="setFilter === 'not-done'" icon="fa-solid fa-check" />
            </a>
          </div>
        </div>
        <button
    ...
    <article data-cy="todo-empty-state" v-else class="w-full p-5 flex justify-center mt-8">
      <img src="../../assets/todo-empty-state.png" alt="empty state" class="w-[300px] sm:w-[600px] mx-auto">
    </article>
  </section>
  <section
  ...
  watch: {
    setFilter(newVal) {
      ...
      switch (newVal) {
        ...
        case 'not-done':
          return this.filteredTodos = this.todos.sort((a, b) => Number(a.is_active) - Number(b.is_active))
      }
    }
  },
  methods: {
